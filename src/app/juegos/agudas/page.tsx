...
const PALABRAS_AGUDAS = [
  { p: "Café", c: true }, { p: "Mesa", c: false }, { p: "León", c: true },
  { p: "Árbol", c: false }, { p: "Perú", c: true }, { p: "Pared", c: true },
  { p: "Cárcel", c: false }, { p: "Reloj", c: true }, { p: "Joven", c: false },
  { p: "Sofá", c: true }, { p: "Amor", c: true }, { p: "Lápiz", c: false }
];

export default function AgudasPage() {
  ...
  const handleSelect = (userChoice: boolean) => {
    const isCorrect = userChoice === PALABRAS_AGUDAS[index].c;
    if (isCorrect) {
      setScore(score + 10);
      setFeedback(true);
    } else {
    ...
    setTimeout(() => {
      setFeedback(null);
      if (index < PALABRAS_AGUDAS.length - 1) {
        setIndex(index + 1);
      } else {
        setFinished(true);
      }
    ...
    return (
      <div className="min-h-screen bg-red-50 p-8 flex items-center justify-center">
        <Card className="max-w-md w-full p-12 text-center rounded-[3rem] shadow-2xl border-4 border-white">
          ...
          <h2 className="text-3xl font-black text-red-600 mb-4">¡Cazador Experto!</h2>
          <p className="text-xl text-gray-600 mb-8">Has dominado las palabras agudas.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 flex flex-col items-center">
      <header className="w-full max-w-4xl flex justify-between items-center mb-12">
        <button onClick={() => router.push('/dashboard')} className="bg-white px-4 py-2 rounded-xl shadow-sm font-black text-red-600">← Salir</button>
        <div className="bg-white px-6 py-2 rounded-full shadow-sm font-black text-red-600">Puntos: {score}</div>
      </header>
      <main className="w-full max-w-2xl text-center">
        <h1 className="text-4xl font-black text-gray-800 mb-4">¿Es una palabra Aguda?</h1>
        <p className="text-gray-500 mb-12 font-bold">(Recuerda: acento en la última sílaba)</p>
        <Card className="p-16 rounded-[3rem] bg-white shadow-xl border-none mb-12">
          <h2 className="text-7xl font-black text-red-600 animate-pulse">{PALABRAS_AGUDAS[index].p}</h2>
        </Card>
